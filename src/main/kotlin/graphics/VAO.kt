...
class VAO(
	...
	class ArrayAttribute(perVertex: Int, val elements: Int): Attribute(perVertex)
	...
	companion object {
		...
		private fun addAttribute(location: Int, attribute: Attribute): Int {
			...
			when (attribute) {
				...
				is ArrayAttribute -> {
					glBufferData(GL_ARRAY_BUFFER, attribute.elements * attribute.perVertex * 4L, GL_DYNAMIC_DRAW)
	...
	fun renderInstanced(numInstances: Int) {
		glBindVertexArray(vao)
		glDrawElementsInstanced(drawMode, indexCount, GL_UNSIGNED_INT, 0, numInstances)
	}
